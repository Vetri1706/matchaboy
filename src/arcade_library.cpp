...
std::filesystem::path materialize(const std::filesystem::path &directory,
                                 const ArcadeGame &game, std::span<const char> bytes) {
    std::filesystem::create_directories(directory);
    const auto file = directory/(std::string(game.id)+(std::string(game.system)=="GB" ? ".gb" : ".gba"));
...
std::filesystem::path arcade_rom_path(std::size_t index) {
    ...
    const auto resource = FindResourceW(module,MAKEINTRESOURCEW(201+index),RT_RCDATA);
    ...
    if (count && count < executable.size()) {
        try {
            return materialize(std::filesystem::path(executable.data()).parent_path()/L"Matchaboy Data"/L"Library",catalog[index],bytes);
    ...
    return materialize(directory,catalog[index],bytes);
}
