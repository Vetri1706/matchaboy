...
void text(Gdiplus::Graphics *context, double x, double y, const std::string &value,
    ...
    Gdiplus::Font font(L"Consolas", static_cast<float>(size), Gdiplus::FontStyleRegular, Gdiplus::UnitPixel);
    Gdiplus::SolidBrush brush(ink(color));
    context->DrawString(string.c_str(), static_cast<INT>(string.size()), &font,
...
struct Imaging {
    ...
    ~Imaging() { Gdiplus::GdiplusShutdown(token); }
};
...
class Runtime {
  public:
    ...
    void toggle_inspector() { if (!gba) inspector_view = !inspector_view; }
    ...
    std::unique_ptr<Gdiplus::Bitmap> render_player(bool include_lcd = true) {
        ...
        text(context, 40, 24, "MATCHABOY", 28, cyan);
        ...
        text(context, 1098, 32, gba ? "GBA player" : "Inspector", 18, muted);
    ...
    std::unique_ptr<Gdiplus::Bitmap> render() {
        if (gba || !inspector_view) return render_player();
        ...
        text(context, 24, 891, "TAB back to game   CTRL+O open game   SPACE pause   S instruction   F frame   D dot   scroll trace   F12 capture", 11, muted);
        return rendered;
    }
    void save(const std::filesystem::path &path, Gdiplus::Bitmap &bitmap, bool gpu) {
        ...
        if (gba) {
            ...
            metadata << format("{\"platform\":\"gba\",\"width\":%u,\"height\":%u,\"gpu_readback\":%s,\"frames\":%llu,\"paused\":%s,\"buttons\":%u,\"inspector_view\":false}\n",
                bitmap.GetWidth(), bitmap.GetHeight(), gpu ? "true" : "false", gba->frames(), paused ? "true" : "false", buttons);
        ...
        const auto state = inspector->snapshot();
        std::ofstream metadata(path.wstring() + L".json");
        metadata << format("{\"width\":%u,\"height\":%u,\"gpu_readback\":%s,\"frames\":%llu,\"cycles\":%llu,\"ly\":%u,\"dot\":%u,\"mode\":%u,\"fifo_depth\":%u,\"instructions\":%llu,\"paused\":%s,\"buttons\":%u,\"trace_scroll\":%u,\"inspector_view\":%s}\n",
            bitmap.GetWidth(), bitmap.GetHeight(), gpu ? "true" : "false", state.frames, state.cycles,
            state.ly, state.dot, state.mode, state.fifo_depth, state.instructions, paused ? "true" : "false", buttons, trace_scroll, inspector_view ? "true" : "false");
        if (!metadata) throw std::runtime_error("cannot write capture metadata");
...
class Window {
  public:
    ...
    bool chrome_valid = false, cached_paused = false, cached_controls = true;
    ...
    void key(WPARAM key, bool down, bool repeated) {
        if (down && !repeated) {
            ...
            else if (key == 'D' && runtime.inspector_view) { runtime.paused = true; runtime.bus->tick(1); }
    ...
    void paint() {
        ...
        const bool refresh = runtime.inspector_view || !chrome_valid || cached_title != runtime.title || cached_paused != runtime.paused || cached_controls != runtime.controls_visible;
        if (refresh) {
        auto rendered = runtime.inspector_view ? runtime.render() : runtime.render_player(false);
        ...
        chrome_valid = !runtime.inspector_view;
        ...
        if (!runtime.inspector_view) {
            ...
            const float left = static_cast<float>(runtime.lcd_left())/canvas_width, right = static_cast<float>(runtime.lcd_left()+800)/canvas_width;
            const float top = 1-static_cast<float>(runtime.lcd_top())/canvas_height, bottom = 1-static_cast<float>(runtime.lcd_top()+runtime.display_height())/canvas_height;
            glBegin(GL_QUADS);
    ...
    static LRESULT CALLBACK procedure(HWND hwnd, UINT message, WPARAM wp, LPARAM lp) {
        ...
        try {
            ...
            if (!self->window_test) MessageBoxW(hwnd, wide(error.what()).c_str(), L"Silicon Autopsy", MB_OK | MB_ICONERROR);
            PostQuitMessage(1);
            return 0;
        }
    ...
    int run(HINSTANCE instance) {
        ...
        AppendMenuW(view_menu, MF_STRING, 1003, L"Player palette: &Grayscale");
        AppendMenuW(view_menu, MF_STRING, 1004, L"Player palette: Original &green");
...
int WINAPI wWinMain(HINSTANCE instance, HINSTANCE, PWSTR, int) {
    ...
    try {
        ...
        runtime.inspector_view = inspector_view && !runtime.gba;
        ...
        std::cerr << "autopsy: " << error.what() << '\n';
        if (!headless && !window_test) MessageBoxW(nullptr, wide(error.what()).c_str(), L"Silicon Autopsy", MB_OK | MB_ICONERROR);
        return 1;
    }
}
